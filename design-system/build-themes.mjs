...
// ---- Theme definitions (only the five in use) -----------------------------
const THEMES = [
  ...
  { id: 'amber-commerce',  name: 'Amber Commerce',     primary: 28,  secondary: 200, accent: 14,  neutral: 30,  sat: 78, products: ['POS', 'E-Commerce'] },
...
// Target luminances, chosen once and applied to every theme so all five feel
...
const build = t => {
  const light = {
    sunk: atLuminance(t.neutral, 24, L_TARGET.sunk),
    base: atLuminance(t.neutral, 30, L_TARGET.base),
    surface: '#ffffff',
    raised: '#ffffff',
    border: atLuminance(t.neutral, 20, L_TARGET.border),
    borderStrong: atLuminance(t.neutral, 18, L_TARGET.borderStrong),
  };
  const dark = {
    sunk: atLuminance(t.neutral, 32, D_TARGET.sunk),
    base: atLuminance(t.neutral, 30, D_TARGET.base),
    surface: atLuminance(t.neutral, 28, D_TARGET.surface),
    raised: atLuminance(t.neutral, 26, D_TARGET.raised),
    border: atLuminance(t.neutral, 20, D_TARGET.border),
    borderStrong: atLuminance(t.neutral, 18, D_TARGET.borderStrong),
  };

  const solveOn = (bg, h, s, target, ascending) => solveAgainstAll(h, s, [bg], target, ascending);

  light.text = solveOn(light.surface, t.neutral, 30, 12, false);
  light.textSecondary = solveOn(light.surface, t.neutral, 18, 7, false);
  light.textMuted = solveOn(light.surface, t.neutral, 14, 4.6, false);
  ...
  dark.text = solveOn(dark.surface, t.neutral, 26, 12, true);
  dark.textSecondary = solveOn(dark.surface, t.neutral, 18, 7, true);
  dark.textMuted = solveOn(dark.surface, t.neutral, 14, 4.6, true);
